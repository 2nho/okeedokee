						...
						<div id="board">
							...
							<div id="pagingBox">
		                        ...
		                        <c:choose>
		                        	...
		                        	<c:otherwise><a href="${pageContext.request.contextPath}/witnessing?num=${minBlock-1}${query}" class="paging">◀◀</a></c:otherwise>
		                        </c:choose>
								&nbsp;&nbsp;&nbsp;
								<!-- 왼쪽 페이지 이동 -->
								<c:choose>
									<c:when test="${num == 1}"></c:when>
									<c:otherwise><a href="${pageContext.request.contextPath }/witnessingnum=${num-1}${query}" class="paging">◀</a></c:otherwise>
								</c:choose>
								&nbsp;&nbsp;
								<!-- 페이지별 선택 이동 -->
								<c:forEach begin="${minBlock}" end="${(total < maxBlock)? total:maxBlock }" step="1" var="i">
									<c:choose>
										...
										<c:otherwise><a href="${pageContext.request.contextPath}/witnessing?num=${i}${query}">&nbsp;${i}&nbsp;</a></c:otherwise>
									</c:choose>
								</c:forEach>
								&nbsp;&nbsp;
								<!-- 오른쪽 페이지 이동 -->
								<c:choose>
									...
									<c:otherwise><a href="${pageContext.request.contextPath }/witnessing?num=${num+1}${query}" class="paging">▶</a></c:otherwise>
								</c:choose>
								&nbsp;&nbsp;&nbsp;
								<!-- 오른쪽 페이지 블럭 이동 -->
								<c:choose>
									<c:when test="${maxBlock >= total }"></c:when>
									<c:otherwise><a href="${pageContext.request.contextPath}/witnessing?num=${maxBlock+1}${query}" class="paging">▶▶</a></c:otherwise>
								</c:choose>
							</div>
							<!-- 검색 -->
							<div class="searchBox">
